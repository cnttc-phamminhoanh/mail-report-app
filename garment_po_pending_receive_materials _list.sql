...
SELECT 
	'GI' AS [Acc],
	case when left(po2.goods_no,2)='M1' THEN '服装主料(Main material (Fabric)' 
     when left(po2.goods_no,2)='M2' THEN '服装辅料(Sewing material)' 
	 when left(po2.goods_no,2)='M3' THEN '服装包材(Packaging material)'
	end as [Catogery],
...
WHERE 
	...
	AND po1.close_sw = '0' 
	AND po2.close_sw = '0'
	AND po2.goods_no <> 'M164000100001'  -- 附加费
